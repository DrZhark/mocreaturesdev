...
public class EntityAIMoverHelperMoC extends EntityMoveHelper 
{
    ...
    private void floating() {
        ...
        {
            EntityPlayer ep = (EntityPlayer) theCreature.riddenByEntity;
            if (ep.isAirBorne) // TODO TEST
            {
                System.out.println("player is airborne");
                theCreature.motionY += 0.09D;
            } else {
                theCreature.motionY = -0.008D;
            }
            return;
        }

        if ((theCreature.getAttackTarget() != null && ((theCreature.getAttackTarget().posY < (this.posY - 0.5D)) && theCreature.getDistanceToEntity(theCreature.getAttackTarget()) < 10F))) {
